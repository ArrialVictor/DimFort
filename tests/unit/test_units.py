...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("kg/(m*s)", (1, -1, -1, 0, 0, 0, 0), 1),
        # Canonical ``^`` precedence: binds tighter than ``/`` so
        # ``m/s^2`` is ``m/(s^2)``, not ``(m/s)^2``. (The same
        # precedence applies to ``**`` under the optional
        # ``allow_fortran_star_star`` flag — covered in
