...
| H020  | error    | Polymorphic call-site unification failure: two argument slots imply inconsistent units for the same type variable. The message lists every contributing slot with its implied binding and a symmetric `(collides with arg N: name)` trailer. See [polymorphism.md](polymorphism.md). |
